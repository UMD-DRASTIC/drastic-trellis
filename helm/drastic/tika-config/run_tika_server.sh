...
URL="https://opennlp.sourceforge.net/models-1.5"
...
if [ "$(ls -A $OPENNLP_LOCATION/*.bin)" ]; then
	...
	wget -v -d \"$URL/en-ner-person.bin\" -O $OPENNLP_LOCATION/ner-person.bin
...
exec java -Dner.impl.class=$RECOGNISERS -cp $CLASSPATH org.apache.tika.server.core.TikaServerCli -h 0.0.0.0 -noFork -c /config/tika-config.xml
